...
from utils.finance_evaluation import verify_metrics_table_remote, win_join
...
class TaskConfig(GeneralTaskConfig):
    TASK_TAG: str = "ar_challenge_filter_60"
...
2) {self.target_companies_url}
   - JSON array, one object per target company.
   - Required keys: `证券代码`, `股票简称`.
   - Restrict output rows to this target-company list only.
...
Core Rule Set:
- Filter: EducationLevelFixed <= 2, 年龄<=55, 是否兼任董事==2.
- Compute company-level metrics: `Qualified Count`, `Qualified Avg Salary 2019-2023`, `Qualified Avg Shareholding 2024`.
- Row inclusion for target companies:
  - Keep a row only when all are valid: `Qualified Count > 0`, `Qualified Avg Salary 2019-2023` numeric,
    `Qualified Avg Shareholding 2024` numeric.
  - Skip that target company if any required metric is not computable.

Output File:
- Save one Excel file to {self.output_url}
- Required columns (strict order):
  "证券代码", "股票简称", "Qualified Count", "Qualified Avg Salary 2019-2023", "Qualified Avg Shareholding 2024"
